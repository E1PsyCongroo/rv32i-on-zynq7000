...
#define UTRAN_DATA (*((volatile uint32_t*)0x80000008))
  if (URECV_CTRL) return URECV_DATA;
  return 0;
}
...
static void print_head(snake_t* snake, rect_t board) {
  ...
  game_board[p->y+board.top+1][p->x+board.left+1] = '@';
}
...
static void clear_tail(snake_t* snake, rect_t board) {
  ...
  game_board[snake->body[t].y + board.top + 1][snake->body[t].x + board.left + 1] = 0;
}
